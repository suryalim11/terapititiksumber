                        ...
                        <div className="mb-2">
                          <Input 
                            ...
                            onChange={(e) => {
                              ...
                              // Update state untuk digunakan di dropdown
                              ...
                              // Cari pasien yang cocok
                              const matchingPatient = patients.find((patient: Patient) => {
                                if (!patient) return false;
                                
                                // Pastikan semua data dalam format string yang konsisten
                              ...
                              // Auto-select jika ada hasil yang cocok
                              if (matchingPatient) {
                                const patientIdStr = String(matchingPatient.id);
                                console.log("✅ Pasien ditemukan berdasarkan pencarian:", matchingPatient.name);
                                
                                // Tambahan: delay sedikit untuk memastikan UI terupdate dengan tepat
                                setTimeout(() => {
                            ...
                            {patients?.length === 0 ? (
                              ...
                              patients
                                .filter(patient => {
                                  ...
                                  // Pastikan semua nilai dikonversi ke string dengan benar
                                  const patientName = patient.name ? String(patient.name).toLowerCase() : '';
                                  const patientId = patient.patientId ? String(patient.patientId).toLowerCase() : '';
                                  const searchTermLower = searchTerm.toLowerCase().trim();
                                  
                                  // Pencarian berdasarkan nama dan ID pasien (lebih sederhana)
                                  ...
                                  // Pencarian berdasarkan nomor telepon yang dinormalisasi
                                  if (patient.phoneNumber) {
                                    // Fungsi normalisasi nomor telepon (sama dengan yang digunakan di onChange)
                                    const normalizePhoneNumber = (phone: string): string => {
                                      if (!phone) return '';
                                      // Hapus karakter non-numerik
                                      const numericOnly = phone.replace(/\D/g, '');
                                    ...
                                    // Normalisasi nomor telepon untuk pencarian yang lebih akurat
                                    const phoneNumber = String(patient.phoneNumber);
                                    const normalizedPatientPhone = normalizePhoneNumber(phoneNumber);
