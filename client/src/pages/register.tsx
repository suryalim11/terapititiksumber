...
export default function RegisterPage() {
  ...
  useEffect(() => {
    if (therapySlots && therapySlots.length > 0) {
      // Cek apakah ada slotId yang tersimpan di sessionStorage atau di URL
      ...
      if (slotIdToUse) {
        ...
        if (matchingSlot) {
          console.log("Menemukan therapy slot yang cocok dari ID:", matchingSlot);
  ...
  // Fungsi untuk menampilkan slot terapi yang dikelompokkan berdasarkan tanggal
  const renderGroupedSlots = () => {
    if (!therapySlots || therapySlots.length === 0) return null;
